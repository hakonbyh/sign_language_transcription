...
from main.decoders import BERTDecoder, Decoder, RecurrentDecoder, TransformerDecoder
from main.embeddings import Embeddings, SpatialEmbeddings
from main.encoders import BERTEncoder, Encoder, RecurrentEncoder, TransformerEncoder
...
from main.src.models import ResNet18
...
class SignModel(nn.Module):
    ...
    def __init__(
        ...
        logger.info(f"BOS = {self.txt_bos_index}")
        logger.info(f"PAD = {self.txt_pad_index}")
        logger.info(f"EOS = {self.txt_eos_index}")
    ...
    def forward(
        ...
        sgn_dim = self.sgn_embed.input_size

        sgn_videos = []
        sgn_kps = []
        for video, keypoints in sgn:
            sgn_videos.append(video.float())
            sgn_kps.append(keypoints.float())

        sgn_embeds = self.conv2d(sgn_videos)

        sgn = [
            torch.cat((embeds, kps), dim=1) for embeds, kps in zip(sgn_embeds, sgn_kps)
        ]

        sgn = pad_sequence(sgn, batch_first=True, padding_value=0)

        sgn_mask = (sgn != torch.zeros(sgn_dim, device=sgn.device))[..., 0].unsqueeze(1)
    ...
    def run_batch(
        ...
        sgn_videos = []
        sgn_kps = []
        for video, keypoints in batch.sgn:
            sgn_videos.append(video.float())
            sgn_kps.append(keypoints.float())
        ...
        encoder_output, encoder_hidden = self.encode(
            sgn=sgn, sgn_mask=sgn_mask, sgn_length=batch.sgn_lengths
        )
